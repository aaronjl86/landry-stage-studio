...
export const TEMPLATE_PROMPTS = {
  ...
  decluttering:
    "CRITICAL CONSTRAINT: Remove ONLY movable objects - ZERO architectural changes allowed. You MUST preserve the EXACT room structure as photographed. REMOVE ONLY: furniture (sofas, chairs, tables, beds, dressers), personal items (decorations, photos, books, electronics), and movable objects. ABSOLUTELY FORBIDDEN - DO NOT: add windows, remove windows, move windows, change window sizes, alter window positions, add walls, remove walls, move walls, change wall colors/textures, modify ceiling, change floor surface, add doors, remove doors, change moldings, alter built-in shelves/cabinets, modify light fixtures, change room dimensions, add or remove architectural features. WINDOWS MUST REMAIN EXACTLY AS PHOTOGRAPHED - same number, same size, same position, same frames, same glass. The result must be a clean EMPTY room with 100% preservation of all walls, windows, doors, floors, ceiling, moldings, built-ins, and every permanent architectural feature exactly as photographed. Architecture must be photographically identical to the original.",
  "virtual-staging":
