-- Phase 1: Create signup tracking table
...
CREATE INDEX idx_signup_ip ON signup_attempts(ip_address, created_at);
CREATE INDEX idx_signup_device ON signup_attempts(device_fingerprint, created_at);
CREATE INDEX idx_signup_email_domain ON signup_attempts(email_domain, created_at);
CREATE INDEX idx_signup_status ON signup_attempts(status);

-- Enable RLS
ALTER TABLE public.signup_attempts ENABLE ROW LEVEL SECURITY;

-- Admin-only access policy (will need admin role system)
...
CREATE TABLE public.blacklisted_identifiers (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_blacklist_lookup ON blacklisted_identifiers(identifier_type, identifier_value);
...
ALTER TABLE public.profiles 
  ...
  ADD COLUMN IF NOT EXISTS risk_level text DEFAULT 'low' CHECK (risk_level IN ('low', 'medium', 'high', 'blocked')),
...
BEGIN
  ...
  -- Count recent signups from same IP (last 24 hours)
  ...
  -- Count signups from same device (last 7 days)
  ...
  -- Count signups from same email domain (last hour)
...
-- Phase 6: Update handle_new_user trigger to capture metadata
...
BEGIN
  ...
  -- Try to extract IP and device fingerprint from metadata
  BEGIN
  ...
  -- Insert profile with tracking data
